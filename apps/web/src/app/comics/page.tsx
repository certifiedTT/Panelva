...
export default function ComicsPage() {
  ...
  const comics: Comic[] = useMemo(() => [
    { 
      id: "1", 
      title: "Love Bites", 
      author: "Lee Jehwan", 
      genre: "Romance",
      format: "Visual Comics",
      status: "Ongoing", 
      desc: "Step into a world of visual romance and sweet bloodthirsty bites.", 
      coverBg: "linear-gradient(135deg, #4c0519, #9f1239)",
      likes: "3.9M",
      isNew: true,
      chapters: 42
    },
    { 
      ...
      format: "Webtoon",
      status: "Ongoing", 
      desc: "Catching stars and chasing hearts in the dark night sky.", 
      coverBg: "linear-gradient(135deg, #1e1b4b, #311042)",
      likes: "3M",
      isNew: true,
      chapters: 36
    },
    { 
      id: "3", 
      title: "A Spell for a Smith", 
      author: "MagusSmith", 
      genre: "Fantasy",
      format: "Visual Comics",
      status: "Ongoing", 
      desc: "A blacksmith receives a magical spell that changes his forging forever.", 
      coverBg: "linear-gradient(135deg, #022c22, #064e3b)",
      likes: "3.4M",
      isNew: true,
      chapters: 48
    },
    { 
      ...
      author: "BarbarianKing", 
      genre: "Fantasy",
      format: "Manga",
      status: "Ongoing", 
      desc: "Trapped inside a game as a weak barbarian, survival is the only option.", 
      coverBg: "linear-gradient(135deg, #450a0a, #781c1c)",
      likes: "4.4M",
      isNew: true,
      chapters: 78
    },
    { 
      ...
      format: "Webtoon",
      status: "Ongoing", 
      desc: "Working out and finding love in the local community center.", 
      coverBg: "linear-gradient(135deg, #1f2937, #111827)",
      likes: "7.6M",
      isNew: true,
      chapters: 88
    },
    { 
      id: "6", 
      title: "Sweet Romance, Spicy Roommates", 
      author: "SpiceWriter", 
      genre: "Romance",
      format: "Visual Comics",
      status: "Ongoing", 
      desc: "Two roommates with spicy secrets find sweet romance.", 
      coverBg: "linear-gradient(135deg, #831843, #db2777)",
      likes: "92,054",
      isNew: true,
      chapters: 24
    }
  ...
  const { data: dbComics } = trpc.series.getMany.useQuery({
    type: "COMIC",
  ...
  const mapDbItem = (item: any): Comic => {
    ...
    return {
      ...
      genre: item.genre,
      format: item.type === "COMIC" ? "Visual Comics" : "Web Novel",
  ...
  const filteredComics = useMemo(() => {
    return comics.filter(comic => {
      const matchesSearch = comic.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            ...
                            comic.desc.toLowerCase().includes(searchQuery.toLowerCase());
      
      const matchesFormat = selectedFormat === "All Formats" || comic.format === selectedFormat;
      const matchesGenre = selectedGenre === "All Genres" || comic.genre === selectedGenre;
      const matchesStatus = selectedStatus === "All Statuses" || comic.status === selectedStatus;

      return matchesSearch && matchesFormat && matchesGenre && matchesStatus;
    });
  }, [comics, searchQuery, selectedFormat, selectedGenre, selectedStatus]);

  const displayList: Comic[] = dbComics && dbComics.length > 0
    ? dbComics.map(mapDbItem)
    : filteredComics;
    ...
    <div className="explore-page" style={{ minHeight: "100vh", backgroundColor: "#08090c", color: "var(--text-dark)", padding: "4rem 2rem", boxSizing: "border-box", fontFamily: "var(--font-sans)", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", flexWrap: "wrap", gap: "1.5rem" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <span style={{ fontSize: "1.1rem", color: "var(--text-dark-muted)" }}>Discover your next favorite visual journey.</span>
        </div>
      ...
      {displayList.length === 0 ? (
        ...
        <div className="explore-items-grid">
          {displayList.map((comic: Comic) => (
            <Link key={comic.id} href={`/read/${comic.id}`} style={{ textDecoration: "none", display: "flex", flexDirection: "column", gap: "10px", width: "100%", minWidth: 0 }}>
              <div style={{
                position: "relative",
                width: "100%",
                aspectRatio: "3/4",
                borderRadius: "12px",
                background: comic.coverBg,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "2px", paddingLeft: "2px" }}>
                <span style={{ fontSize: "0.72rem", color: "#9ca3af", textTransform: "capitalize" }}>
                  {comic.genre} {comic.chapters && `• ${comic.chapters} Chs`}
                </span>
