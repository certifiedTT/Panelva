...
export default function ProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"overview" | "comments" | "wallet">("overview");
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "2.5rem" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1.5rem", marginTop: "1rem" }}>
            ...
            <div 
              className="glass-panel hover:border-blue-500/40 cursor-pointer transition-colors" 
              onClick={() => setActiveTab("wallet")}
              style={{ padding: "1.25rem 1.5rem", display: "flex", alignItems: "center", gap: "14px", background: "#07080a", border: "1px solid #1c1e24", borderRadius: "12px" }}
            >
              ...
              <div>
                <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "#fff" }}>{creditBalance}</div>
                <span style={{ fontSize: "0.7rem", color: "var(--text-dark-muted)", fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase" }}>CREDITS</span>
              </div>
            </div>
          </div>
        ...
        <div style={{ display: "inline-flex", background: "#0d0e12", border: "1px solid #1c1e24", padding: "5px", borderRadius: "30px", gap: "6px", width: "100%", overflowX: "auto" }}>
          {[
            ...
            { id: "wallet", label: "Credits Wallet", icon: <Coins size={15} /> },
        ...
        {activeTab === "comments" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            {comments.map((c: any) => (
              <div key={c.id} className="glass-panel" style={{ padding: "1.5rem", borderRadius: "12px", border: "1px solid #1c1e24" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                  <span style={{ fontSize: "0.85rem", fontWeight: "bold", color: "var(--secondary)" }}>{c.series} (Chapter {c.chapterIndex})</span>
                  <span style={{ fontSize: "0.75rem", color: "var(--text-dark-muted)" }}>{c.date}</span>
                </div>
                <p style={{ margin: 0, fontSize: "0.9rem", color: "#fff" }}>{c.text}</p>
              </div>
            ))}
            {comments.length === 0 && (
